...
export const generateImage = async (req, res) => {
    try {
        ...
        console.log('Attempting to call ClipDrop API...');
        console.log('API Key length:', process.env.CLIPDROP_API?.length);
        console.log('API Key first 10 chars:', process.env.CLIPDROP_API?.substring(0, 10));
        ...
        console.error('API Key being used:', process.env.CLIPDROP_API ? 'Key is set' : 'Key is missing');
        ...
        res.status(500).json({ success: false, message: "Error generating image. Please check server logs." });
    }
};
